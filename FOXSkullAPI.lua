...
local FOXJson = findScript("FOXJson$")
if FOXJson then FOXJson = require(FOXJson) end
...
local parseJsonFOX = FOXJson and FOXJson.decode or parseJson
local toJsonFOX = FOXJson and FOXJson.encode or toJson
...
function anyClass:getItemStack(count, damage)
	...
	priv.generatedItem:setTextures(__sign, gsplit(toJsonFOX(priv.vars), 32764 * 0.75))
...
local function remove(key, entity)
	...
	if FOXJson then FOXJson.removeTextures(self[1].vars) end
...
local function skullTick()
	...
	if selected then
		...
		local str = priv.isErrored and priv.tooltip
